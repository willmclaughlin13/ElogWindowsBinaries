...
/* SR: make regex.c compile under Visual .NET */
      ...
      case charset_not:
         {
            ...
            printf("/charset%s", (re_opcode_t) * (p - 1) == charset_not ? "_not" : "");
...

         {
            ...
            if (many_times_ok) {        /* More than one repetition is allowed, so put in at the
               ...
               if (TRANSLATE(*(p - 2)) == TRANSLATE('.')
                   && zero_times_ok && p < pend && TRANSLATE(*p) == TRANSLATE('\n')
            ...
            GET_BUFFER_SPACE(3);
            INSERT_JUMP(keep_string_p ? on_failure_keep_string_jump : on_failure_jump, laststart, b + 3);
            pending_exact = 0;
            b += 3;
      ...
      case '\\':
         ...
         case '(':
          ...
          handle_open:
            ...
            COMPILE_STACK_TOP.fixup_alt_jump = fixup_alt_jump ? fixup_alt_jump - bufp->buffer + 1 : 0;
...
int stop;
{
   ...
   if (bufp->re_nsub) {
      ...
      regstart = regend = old_regstart = old_regend = best_regstart = best_regend = reg_dummy = NULL;
      ...
      if (p == pend) {          /* End of pattern means we might have succeeded.  */
         ...
         if (d != end_match_2) {
            ...
            if (!FAIL_STACK_EMPTY()) {  /* More failure points to try.  */
               ...
               if (!best_regs_set || (same_str_p && d > match_end)
      ...
      case duplicate:
         {
            ...
            for (;;) {
               ...
               /* Compare that many; failure if mismatch, else move
                  past them.  */
               if (translate ? bcmp_translate((unsigned char *) d, (unsigned char *) d2, mcnt, translate)
                   : bcmp(d, d2, mcnt))
                  goto fail;
               d += mcnt, d2 += mcnt;
            }
      ...
      case pop_failure_jump:
         {
            ...
            POP_FAILURE_POINT(sdummy, pdummy,
                              dummy_low_reg, dummy_high_reg, reg_dummy, reg_dummy, reg_info_dummy);
         }
         /* Note fall through.  */
...
{
   reg_errcode_t ret;
   unsigned syntax = (cflags & REG_EXTENDED) ? RE_SYNTAX_POSIX_EXTENDED : RE_SYNTAX_POSIX_BASIC;
